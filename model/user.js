...
/**
 * minimum length for pass
 */
var password_minlength = 4;
...
module.exports = Nohm.model('User', {
  ...
  properties: {
    ...
    password: {
      ...
      type: function (value, key, old) { // because when typecasting, we create a new salt and hash the pw.
        var pwd, salt, valueDefined = value && typeof(value.length) !== 'undefined';
  ...
  methods: {
        ...
        if (ids.length === 0) {
          ...
          self.load(ids[0], function (err) {
            if (!err && self.p('password') === hasher(password, self.p('salt'))) {
              callback(null, self.profileProperties());
            } else {
    ...
    /**
     * This function makes dealing with user input a little easier, since we don't
     * want the user to be able to do things on certain fields, like the salt.
     * You can specify a data array that might come from the user and an array containing
     * the fields that should be used from used from the data.
     * Optionally you can specify a function that gets called on every field/data pair to
     * do a dynamic check if the data should be included.
     ...
     * the salt field is not included in there. (although we don't
     * have validations for the salt, an empty entry for it would
     * be created in the errors object)
     */
     ...
     * Overwrites nohms allProperties() to make sure password and salt are not given out.
     */
    profileProperties: function (stringify) {
      var properties = this._super_allProperties.call(this);
      ...
      return stringify ? JSON.stringify(profileProps) : profileProps;
    }
  }
});
