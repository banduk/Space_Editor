var restrict    = require('./../controllers/util').restrict,
    occurrences = require('./../controllers/util').occurrences,
    fileGroup   = require('./../controllers/fileGroup'),
    walk        = require('walk'),
    fs          = require('fs'),
  ...
  if(req.query.project && req.query.project.length > 2){
    ...
    try{
      ...
      walker.on("file", function (root, fileStats, next) {
        ...
        var n = fileGroup.usersInGroup[project+"/"+fname];
        if(n){
      ...
      var n = fileGroup.usersInGroup[project];
      if(n){
        filesAndInfo.push(["", n]);
      }else{
        filesAndInfo.push(["", 0]);
